 ...
 * @returns 创建结果
 */
export const createPermission = (data: { name: string; code: string; description?: string }) => {
...
export const setRolePermissions = async (roleId: number, permissionIds: number[]) => {
  ...
  const currentPermissionIds =
    role.data.permissions?.map((p: Permission | string) =>
      typeof p === "string" ? parseInt(p) : p.id,
    ) || [];
